  ...
  <Layout>
    <div class="p-6">
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-gray-900">Rule Sources</h2>
          <p class="text-sm text-gray-500 mt-1">Manage external rule sources and synchronization</p>
        </div>
        <div class="flex gap-2">
          <button @click="syncAll" :disabled="syncing" class="btn-secondary">
            {{ syncing ? 'Syncing...' : 'Sync All' }}
          </button>
          <button @click="showAddModal = true" class="btn-primary">Add Source</button>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div class="px-4 py-3 bg-gray-50 border-b">
            <h3 class="font-medium text-gray-700 text-sm">Built-in Sources</h3>
          </div>
          <div class="divide-y divide-gray-100">
            <div v-for="src in builtinSources" :key="src.name" class="px-4 py-3 flex items-center justify-between">
              ...
              <div class="flex items-center gap-3">
                <span class="text-xs text-gray-500">{{ src.count }} rules</span>
                <span class="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs font-medium">builtin</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div class="px-4 py-3 bg-gray-50 border-b">
            <h3 class="font-medium text-gray-700 text-sm">Configured Sources</h3>
          </div>
          <div v-if="sources.length === 0" class="px-4 py-8 text-center text-gray-400 text-sm">
            No external sources configured. Add one to get started.
          </div>
            ...
            <div v-for="src in sources" :key="src.name" class="px-4 py-3">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-gray-400">{{ src.lastUpdated ? 'Updated ' + formatDate(src.lastUpdated) : 'Never synced' }}</span>
      ...
      <div v-if="showAddModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="showAddModal = false">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
          <div class="px-6 py-4 border-b">
            <h3 class="font-semibold text-gray-900">Add Rule Source</h3>
          </div>
          <div class="px-6 py-4 space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ newSource.type === 'remote_url' ? 'URL *' : 'Path *' }}
              </label>
              <input
          ...
          <div class="px-6 py-4 border-t flex gap-2 justify-end">
            ...
            <button @click="addSource" :disabled="!newSource.name || !newSource.url" class="btn-primary">Add Source</button>
          </div>
        </div>
      </div>
    </div>
