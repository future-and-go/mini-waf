...
/// plan §Security).
pub fn record_sample(recorder: Option<&Arc<Recorder>>, key: &FpKey, path: &str, had_referer: bool, tier: Tier) {
    if let Some(rec) = recorder
        && !key.is_empty()
    {
        rec.record(key, path, had_referer, tier);
    }
}
...
mod tests {
    ...
    fn three_records_produce_three_samples() {
        ...
        let k = key("a");
        for _ in 0..3 {
            record_sample(Some(&rec), &k, "/x", false, Tier::CatchAll);
        }
    ...
    fn empty_key_is_skipped() {
        ...
        record_sample(Some(&rec), &empty, "/x", false, Tier::CatchAll);
    ...
    fn no_recorder_is_noop() {
        ...
        let k = key("a");
        record_sample(None, &k, "/x", false, Tier::CatchAll);
    ...
    fn referer_flag_propagates_to_sample() {
        let rec = make_recorder();
        let k = key("b");
        record_sample(Some(&rec), &k, "/x", true, Tier::CatchAll);
    ...
    fn tier_propagates_to_sample() {
        let rec = make_recorder();
        let k = key("c");
        record_sample(Some(&rec), &k, "/x", false, Tier::Critical);
        let snap = rec.snapshot(&k).expect("snapshot");
