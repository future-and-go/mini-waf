...
pub fn apply_chunk(
    ...
    let raw = body.take().unwrap_or_default();
    state.processed = state.processed.saturating_add(raw.len() as u64);
    ...
    // reject any single chunk that ends mid-block. We MUST NOT treat such a
    // mid-stream error as terminal failure — `DecoderChain::push` retains the
    // unconsumed input internally, so a subsequent push that completes the
    // gzip frame will succeed. Only at `eos = true` do we commit a failure
    // verdict. On any failure, we replace the body with empty bytes (NOT the
    // raw gzip bytes) because `Content-Encoding` was already stripped in
    // `response_filter` — forwarding raw gzip would label as identity and
    // garble the response on the client side.
    let plaintext: Vec<u8> = if let Some(chain) = state.decoder.as_mut() {
        match chain.push(&raw) {
            ...
            Err(e) => {
                if eos {
                    // Terminal: corrupt or truncated gzip stream at end-of-
                    // stream. Replace body with empty (B3 — never garbled raw).
                    tracing::warn!(err = %e, "body-scan: decoder push failed at eos; replacing body with empty");
                    state.failed = true;
                    *body = Some(Bytes::new());
                    return;
                }
                // Mid-stream push error = incomplete gzip frame. Tolerate;
                // the decoder retains the pending input and the next push
                // may complete the frame. Emit empty body for THIS chunk.
                tracing::debug!(err = %e, "body-scan: decoder push deferred (incomplete frame, awaiting more bytes)");
                *body = Some(Bytes::new());
                return;
            }
        }
