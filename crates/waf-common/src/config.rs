...
///
/// The WAF appends one JSON object per processed request to `log_path`
/// (JSONL), created lazily on the first request. This file is the **sole**
/// audit sink — the benchmarker reads it back after each run for correlation
/// and score validation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditFileConfig {
    /// Master switch. When `false`, no audit file is written.
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Audit log path, resolved relative to the process working directory.
    #[serde(default = "default_audit_log_path")]
    pub log_path: String,
}

fn default_audit_log_path() -> String {
    "./waf_audit.log".to_string()
}
