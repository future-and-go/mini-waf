...
fn fr_014_xss_blocks_json_payload() {
    ...
    let d = c.check(&mut req).expect("FR-014 hit");
...
fn fr_015_path_traversal_blocks_double_encoded() {
    ...
    let mut req = ctx("/files/%252e%252e%252fetc%252fpasswd", "GET", b"", "", HashMap::new());
    let d = c.check(&mut req).expect("FR-015 hit");
...
fn fr_016_ssrf_blocks_aws_metadata() {
    ...
    let d = c.check(&mut req).expect("FR-016 hit");
...
fn fr_017_header_injection_blocks_crlf_in_referer() {
    ...
    let mut req = ctx("/", "GET", b"", "", h);
    let d = c.check(&mut req).expect("FR-017 hit");
...
fn fr_018_brute_force_blocks_after_five_failed_logins() {
    ...
    let mut req = ctx("/login", "POST", body, "application/json", HashMap::new());
    let d = c.check(&mut req).expect("FR-018 hit");
...
fn fr_019_scanner_blocks_options_abuse() {
    ...
    for _ in 0..19 {
        let mut r = ctx("/api/users", "OPTIONS", b"", "", HashMap::new());
        let _ = c.check(&mut r);
    }
    let mut req = ctx("/api/users", "OPTIONS", b"", "", HashMap::new());
    let d = c.check(&mut req).expect("FR-019 hit");
...
fn fr_020_body_abuse_blocks_oversized_declared_length() {
    ...
    let d = c.check(&mut req).expect("FR-020 hit");
...
fn fr_clean_benign_request_triggers_no_detection() {
    let mut req = ctx("/api/v1/users/42", "GET", b"", "", HashMap::new());
    assert!(XssCheck::new().check(&mut req).is_none());
    assert!(DirTraversalCheck::new().check(&mut req).is_none());
    assert!(SsrfCheck::new().check(&mut req).is_none());
    assert!(HeaderInjectionCheck::new().check(&mut req).is_none());
    assert!(BruteForceCheck::new().check(&mut req).is_none());
    assert!(RequestBodyAbuseCheck::new().check(&mut req).is_none());
    // ScannerCheck's stateful OPTIONS/enum-path counters fire only under
    // threshold — a single clean GET must not trigger them.
    assert!(ScannerCheck::new().check(&mut req).is_none());
}
