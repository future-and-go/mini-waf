...
### Added
- **§5 Observability Header Injection (Interop Contract v2.3)**: Every HTTP response (block, allow, challenge, cache HIT, error, transport failure) injects six mandatory `X-WAF-*` headers (`X-WAF-Request-Id`, `X-WAF-Risk-Score`, `X-WAF-Action`, `X-WAF-Rule-Id`, `X-WAF-Cache`, `X-WAF-Mode`). Single DRY injector module (`gateway::waf_observability_headers`) with three helpers: `inject_waf_observability_headers` (WAF-decision pages), `inject_for_passthrough[_with_cache]` (allow/MISS/challenge-passed/HIT), `inject_for_pre_inspect_or_error` (pre-WAF paths with fallback UUID v4 for `X-WAF-Request-Id`). Ordering invariant: injected AFTER FR-035 HeaderFilter + cache snapshot capture; headers survive FR-035 and never enter cache. Paired with `WafEngine::emit_minimal_audit_stub` for §5↔§6 audit correlation (commits 59a27dd, 9e4010a, 3816a08).
- **§3 WAF Decision Classes (Interop Contract v2.3)**: Extended `WafAction` enum with `RateLimit(429)`, `Timeout`, `CircuitBreaker` variants; enriched `WafDecision` with `risk_score`, `mode` (enforce/log_only), `rule_id` for decision logging. Log-only mode now preserved via decision mode (not action variant), allowing intended action to survive audit trail. Gateway response handler maps decision class to correct HTTP status (429 for rate-limit, 503 for timeout, 502 for circuit-breaker). `as_contract_str()` emits interop-compliant action strings (commits ce9043d, b44c420, f2666ba, 0eafad9).
